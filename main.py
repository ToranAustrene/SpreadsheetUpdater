...
#documentation at https://cbpro2.readthedocs.io/en/latest/authenticated_client.html
    ...
    defaultEthAccount = default_client.get_account(config['defaultEthID'])
    # print(defaultEthAccount['currency'], defaultEthAccount['balance'])
    defaultBtcAccount = default_client.get_account(config['defaultBtcID'])
    # print(defaultBtcAccount['currency'], defaultBtcAccount['balance'])
    sleep(1)

    btcBtcAccount = bitcoin_client.get_account(config['btcBtcID'])
    # print(btcBtcAccount['currency'], btcBtcAccount['balance'])
    btcSolAccount = bitcoin_client.get_account(config['btcSolID'])
    # print(btcSolAccount['currency'], btcSolAccount['balance'])
    btcAtomAccount = bitcoin_client.get_account(config['btcAtomID'])
    # print(btcAtomAccount['currency'], btcAtomAccount['balance'])
    btcAlgoAccount = bitcoin_client.get_account(config['btcAlgoID'])
    # print(btcAlgoAccount['currency'], btcAlgoAccount['balance'])
    btcDotAccount = bitcoin_client.get_account(config['btcDotID'])
    # print(btcDotAccount['currency'], btcDotAccount['balance'])
    btcXtzAccount = bitcoin_client.get_account(config['btcXtzID'])
    # print(btcXtzAccount['currency'], btcXtzAccount['balance'])
    sleep(1)

    ethEthAccount = etherium_client.get_account(config['ethEthID'])
    # print(ethEthAccount['currency'], ethEthAccount['balance'])
    ethAdaAccount = etherium_client.get_account(config['ethAdaID'])
    # print(ethAdaAccount['currency'], ethAdaAccount['balance'])
    ethLinkAccount = etherium_client.get_account(config['ethLinkID'])
    # print(ethLinkAccount['currency'], ethLinkAccount['balance'])
    ethBatAccount = etherium_client.get_account(config['ethBatID'])
    # print(ethBatAccount['currency'], ethBatAccount['balance'])
    ethManaAccount = etherium_client.get_account(config['ethManaID'])
    # print(ethManaAccount['currency'], ethManaAccount['balance'])
    ethSushiAccount = etherium_client.get_account(config['ethSushiID'])
